# MSFT — FY2026 Azure AI — Q2 Follow-Up

| Field | Value |
| --- | --- |
| ID | `80a4fc8a-6fe0-4660-98ea-9ebb0ccfdd13` |
| Status | **OPEN** |
| Rating | **BUY** |
| Price Target | N/A |
| Version | v2 |
| Created | 2026-04-25 |

## Thesis

Q2 FY2026 results validated the Azure AI thesis. Azure grew 35% — above our 32% threshold. AI run rate tracking toward $20B. Raising our Azure growth assumption to 38% and maintaining a constructive stance with tighter falsification conditions.

## Assumptions

| Key | Label | Value | Unit | Confidence | Materiality | Status | Version |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `azure_revenue_growth` | Azure Revenue Growth (YoY %) | 0.38 | % | 82% | CRITICAL | ACTIVE | v2 (1 changes) |
| `ai_run_rate_usd_b` | AI Revenue Annualized Run Rate (USD B) | 20.0 | USD B | 80% | HIGH | ACTIVE | v1 |

## Predictions

| Metric | Description | Threshold | Op | Horizon | Due Date | Prob | Status |
| --- | --- | --- | --- | --- | --- | --- | --- |
| `msft_azure_q3_fy2026_yoy` | Azure revenue grows at ≥35% YoY in Q3 FY2026 (Apr 2026 repor… | 0.35 % | >= | Q3 FY2026 earnings (Apr 2026) | 2026-05-15 | 65% | ⏳ PENDING |
| `msft_ai_run_rate_q4_fy2026` | AI run rate exceeds $22B by Q4 FY2026… | 22.0 USD B | >= | Q4 FY2026 (Jun 2026) | 2026-08-15 | 55% | ⏳ PENDING |

_Generated 2026-04-25 16:38 UTC_